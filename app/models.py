...
class User(db.Model):
	...
	userlogs = db.relationship('Userlog', backref='user')  # 会员-会员登录日志 一对多关系
	collect = db.relationship('Collect', backref='user')  #  会员-收藏 一对多关系
...
class Admin(db.Model):
	...
	id = db.Column(db.Integer, primary_key=True)  # 编号
	name = db.Column(db.String(100), unique=True)  # 管理员账号名
	pwd = db.Column(db.String(100))  # 管理员密码

	adminlogs = db.relationship("Adminlog", backref='admin')  # 管理员-管理员登录日志 一对多关系
	oplogs = db.relationship("Oplog", backref='admin')  # 管理员-操作日志  一对多关系
...
class Area(db.Model):
	...
	scenic = db.relationship("Scenic", backref='area')  # 地区-景区 一对多关系
...
class Scenic(db.Model):
	...
	# 设置外键
	area_id = db.Column(db.Integer, db.ForeignKey('area.id'))  # 所属地区id
	collect = db.relationship("Collect", backref='scenic')  # 景区-收藏 一对多关系
	travels = db.relationship("Travels", backref='scenic')  # 景区-游记 一对多关系

	def __repr__(self):
		return f"<Scenic {self.title}>"
